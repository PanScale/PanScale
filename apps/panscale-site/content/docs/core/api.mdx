...
- `scrollToWithZoom(left, top, zoom, animate?)` — Scroll and zoom in one motion
- `setZoomBounds(min, max)` — Update zoom limits at runtime (re-clamps current zoom)
- `fitToContent(animate?)` — Fit content to viewport, centering it
...
interface ScalerValues {
  ...
  scrollLeft: number;
  scrollTop: number;
  isInteracting: boolean;  // true during touch/momentum/bounce
}
```
